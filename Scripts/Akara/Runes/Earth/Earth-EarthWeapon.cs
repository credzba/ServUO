...
{
	public class EarthWeapon : RuneSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
				"Earth Weapon", "Cadre Iquan",
				212,
				9041,
				GetSpellGems.GetGems( typeof(EarthWeapon) )
			);
		...
		public EarthWeapon( Mobile caster, Item scroll, SpellRune rune ) : base( caster, scroll, m_Info, rune )
		{
		}
		...
		public void Target( Mobile m )
		{
			...
			else if ( !m.CanBeginAction( typeof(EarthWeapon) ))
			{
			...
			else if ( CheckSequence() )
			{
				...
				{
					GainExp();
					m.FixedParticles( 14154, 10, 30, 5052, 0, 0, EffectLayer.LeftFoot );
					m.PlaySound( 696 );
					...
					m.BeginAction( typeof( EarthWeapon ) );
					...
					else
					{
						...
						if ( mods == null )
						{
							mods = new ResistanceMod[1]
								{
									new ResistanceMod( ResistanceType.Physical, Rune.Level + (int)(Caster.Skills[SkillName.Inscribe].Value/5) )
								};
		...
		private class Internal2Timer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				if ( mods != null )
				{
					Source.EndAction( typeof( EarthWeapon ) );
		...
		private class InternalHammer : BaseBashing
		{
			...
			{
				pois = nrgy = fire = cold = chaos = direct = 0;
				phys = 100;
			}
			...
			public override void OnDelete()
			{
				...
				m_Owner.EndAction( typeof( EarthWeapon ) );
			...
			public override void OnHit( Mobile attacker, IDamageable m, double damageBonus )
			{
                ...
                ((Mobile)m).SendMessage( "Your body is slashed with pieces of stones." );
				...
				SpellHelper.Damage( TimeSpan.Zero, m, attacker, damage, 100, 0, 0, 0, 0 );
				m.FixedParticles( 14154, 10, 30, 5052, 0, 0, EffectLayer.LeftFoot );
				m.PlaySound( 696 );
			}
		...
		{
			private EarthWeapon m_Owner;

			public InternalTarget( EarthWeapon owner ) : base( 12, true, TargetFlags.None )
			{
				m_Owner = owner;
			}

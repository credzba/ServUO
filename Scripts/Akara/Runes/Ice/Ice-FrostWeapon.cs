...
{
	public class FrostWeapon : RuneSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
				"Frost Weapon", "Iwne Iquan",
				212,
				9041,
				GetSpellGems.GetGems( typeof(FrostWeapon) )
			);
		...
		public FrostWeapon( Mobile caster, Item scroll, SpellRune rune ) : base( caster, scroll, m_Info, rune )
		{
		}
		...
		public void Target( Mobile m )
		{
			...
			else if ( !m.CanBeginAction( typeof(FrostWeapon) ))
			{
			...
			else if ( CheckSequence() )
			{
				if ( m.Player )
				{
					...
					m.BeginAction( typeof( FrostWeapon ) );
					...
					else
					{
						...
						if ( mods == null )
						{
							mods = new ResistanceMod[1]
								{
									new ResistanceMod( ResistanceType.Cold, Rune.Level + (int)(Caster.Skills[SkillName.Inscribe].Value/5) )
								};
		...
		private class Internal2Timer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				if ( mods != null )
				{
					Source.EndAction( typeof( FrostWeapon ) );
		...
		private class InternalHammer : BaseBashing
		{
			...
			{
				phys = nrgy = fire = pois = chaos = direct = 0;
				cold = 100;
			}
			...
			public override void OnDelete()
			{
				...
				m_Owner.EndAction( typeof( FrostWeapon ) );
			...
			public override void OnHit( Mobile attacker, IDamageable m, double damageBonus )
			{
                ...
                ((Mobile)m).SendMessage( "Your body has been stabbed by small pieces of ice." );
				...
				SpellHelper.Damage( TimeSpan.Zero, m, attacker, damage, 0, 0, 100, 0, 0 );
				m.FixedParticles( 14201, 10, 30, 5052, 0, 0, EffectLayer.LeftFoot );
				m.PlaySound( 696 );
			}
		...
		{
			private FrostWeapon m_Owner;

			public InternalTarget( FrostWeapon owner ) : base( 12, true, TargetFlags.None )
			{
				m_Owner = owner;
			}

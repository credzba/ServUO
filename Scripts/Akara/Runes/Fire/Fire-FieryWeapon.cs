...
{
	public class FieryWeapon : RuneSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
				"Fiery Weapon", "Phoe Iquan",
				212,
				9041,
				GetSpellGems.GetGems( typeof(FieryWeapon) )
			);
		...
		public FieryWeapon( Mobile caster, Item scroll, SpellRune rune ) : base( caster, scroll, m_Info, rune )
		{
		}
		...
		public void Target( Mobile m )
		{
			...
			else if ( !m.CanBeginAction( typeof(FieryWeapon) ))
			{
			...
			else if ( CheckSequence() )
			{
				if ( m.Player )
				{
					...
					m.BeginAction( typeof( FieryWeapon ) );
					...
					else
					{
						...
						if ( mods == null )
						{
							mods = new ResistanceMod[1]
								{
									new ResistanceMod( ResistanceType.Fire, Rune.Level + (int)(Caster.Skills[SkillName.Inscribe].Value/5) )
								};
		...
		private class Internal2Timer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				if ( mods != null )
				{
					Source.EndAction( typeof( FieryWeapon ) );
		...
		private class InternalHammer : BaseBashing
		{
			...
			public override void OnDelete()
			{
				...
				m_Owner.EndAction( typeof( FieryWeapon ) );
			...
			public override void OnHit( Mobile attacker, IDamageable m, double damageBonus )
			{
                ...
                ((Mobile)m).SendMessage( "Your body is surrounded by incinerating flames." );
				...
				SpellHelper.Damage( TimeSpan.Zero, m, attacker, damage, 0, 100, 0, 0, 0 );
				m.FixedParticles( 0x3709, 10, 30, 5052, 0, 0, EffectLayer.LeftFoot );
				m.PlaySound( 0x208 );
			}
		...
		{
			private FieryWeapon m_Owner;

			public InternalTarget( FieryWeapon owner ) : base( 12, true, TargetFlags.None )
			{
				m_Owner = owner;
			}

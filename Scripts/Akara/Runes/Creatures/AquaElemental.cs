...
{
	[CorpseName( "an aqua elemental corpse" )]
	public class AquaElemental : BaseCreature
	{
		...
		[Constructable]
		public AquaElemental ( RuneSpell m_Spell, Mobile caster, Mobile source, SpellRune rune ) : base( AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			Name = "Aqua Elemental";
			Body = 16;
			BaseSoundID = 278;
			...
			SetDamageType( ResistanceType.Fire, i6+RSCA.CalculateSI(Caster, i6) );
		...
		public override void OnDelete()
		{
			try
			{
				if ( Source != null )
				{
					Source.SendMessage( "The aqua elemental dissipates." );
					bool lastone = true;
					foreach( Mobile m in World.Mobiles.Values )
					{
						if ( m is AquaElemental )
						{
							AquaElemental ae = (AquaElemental)m;
							if ( ae != this && ( ae.Source == Source || ae.ControlMaster == Source || ae.SummonMaster == Source ) )
								lastone = false;
						}
					}
					if ( lastone )
						Source.EndAction( typeof( AquaElementalSpell ) );
				}
				base.OnDelete();
			}
			catch ( Exception e )
			{
				Console.WriteLine("Aqua Elemental Endeffect Crash Prevented: "+e);
			}
		}
		...
		public AquaElemental( Serial serial ) : base( serial )
		{
		}
		...
		private class ReleaseEntry : ContextMenuEntry
		{
			...
			private AquaElemental m_AE;

			public ReleaseEntry( Mobile from, AquaElemental ae ) : base( 6118, 14 )
			{
				m_From = from;
				m_AE = ae;
			}
			...
			{
				if ( !m_AE.Deleted && m_AE.Controlled && m_From == m_AE.ControlMaster && m_From.CheckAlive() )
					m_AE.BeginRelease( m_From );
			}
		}
	}
}

...
{
	public class PlagueSpore : RuneSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
				"Plague Spore", "Tose Ertl",
				215,
				9041,
				false,
				GetSpellGems.GetGems( typeof(PlagueSpore) )
			);
		...
		public PlagueSpore( Mobile caster, Item scroll, SpellRune rune ) : base( caster, scroll, m_Info, rune )
		{
		}
		...
		public override void OnCast()
		{
			...
			Caster.SendMessage( "Target a location to summon this spore to." );
		}
		...
		private class InternalItem : Item
		{
			...
			public InternalItem( RuneSpell spell, SpellRune rune, Point3D loc, Mobile caster, Map map, TimeSpan duration ) : base( 4518 )
			{
				m_Spell = spell;
				Name = "plague spore";
			...
			public override void OnMovement( Mobile m, Point3D oldLocation )
			{
				...
				if ( Utility.InRange( Location, m.Location, Rune.Level ) && !Utility.InRange( Location, oldLocation, Rune.Level ) )
				{
					if ( m_Caster != m && SpellHelper.ValidIndirectTarget( m_Caster, m ) && m_Caster.CanBeHarmful( m, false ) )
					{
						...
						int total = (int)(runelevel*m_Caster.Skills[SkillName.Inscribe].Value);

						int distx = m.X-this.X;
						distx *= distx;
						int disty = m.Y-this.Y;
						disty *= disty;
						...
						if ( dist >= (int)(runelevel-(runelevel/2)) )
							total -= (int)((dist - (double)(runelevel-(runelevel/2))) * 50);

						total += RSCA.CalculateSI(m_Caster, total);

						total += ImperialOrb.IncI( m_Caster, m_Spell );
						...
						if ( map != null && caster != null )
						{
							...
							while ( m_Queue.Count > 0 )
							{
								...
								if ( m != caster )
								{
									...
									int distx = m.X-m_Item.X;
									distx *= distx;
									int disty = m.Y-m_Item.Y;
									disty *= disty;
		...
		{
			private PlagueSpore m_Owner;

			public InternalTarget( PlagueSpore owner ) : base( 12, true, TargetFlags.None )
			{
				m_Owner = owner;
			}

...
{
	public class ToxicWeapon : RuneSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
				"Toxic Weapon", "Tose Iquan",
				212,
				9041,
				GetSpellGems.GetGems( typeof(ToxicWeapon) )
			);
		...
		public ToxicWeapon( Mobile caster, Item scroll, SpellRune rune ) : base( caster, scroll, m_Info, rune )
		{
		}
		...
		public void Target( Mobile m )
		{
			...
			else if ( !m.CanBeginAction( typeof(ToxicWeapon) ))
			{
			...
			else if ( CheckSequence() )
			{
				...
				{
					GainExp();
					m.FixedParticles( 4518, 10, 30, 5052, 0, 0, EffectLayer.LeftFoot );
					m.PlaySound( 696 );
					...
					m.BeginAction( typeof( ToxicWeapon ) );
					...
					else
					{
						...
						if ( mods == null )
						{
							mods = new ResistanceMod[1]
								{
									new ResistanceMod( ResistanceType.Poison, Rune.Level + (int)(Caster.Skills[SkillName.Inscribe].Value/5) )
								};
		...
		private class Internal2Timer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				if ( mods != null )
				{
					Source.EndAction( typeof( ToxicWeapon ) );
		...
		private class InternalHammer : BaseBashing
		{
			...
			{
				phys = nrgy = fire = cold = chaos = direct = 0;
				pois = 100;
			}
			...
			public override void OnDelete()
			{
				...
				m_Owner.EndAction( typeof( ToxicWeapon ) );
            ...
            // Felix this needs to be changed to use new system
			public /*override*/ void OnHit( Mobile attacker, Mobile m, double damageBonus )
			{
				if ( Rune == null )
				{
					Delete();
				...
				m.SendMessage( "Your body is surrounded by an intoxicating gas." );
				...
				SpellHelper.Damage( TimeSpan.Zero, m, attacker, damage, 0, 0, 0, 100, 0 );
				m.FixedParticles( 4518, 10, 30, 5052, 0, 0, EffectLayer.LeftFoot );
				m.PlaySound( 696 );
			}
		...
		{
			private ToxicWeapon m_Owner;

			public InternalTarget( ToxicWeapon owner ) : base( 12, true, TargetFlags.None )
			{
				m_Owner = owner;
			}
